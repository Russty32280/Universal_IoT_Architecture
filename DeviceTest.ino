...
const int PIN_RED = 25;    // Red
const int PIN_GREEN = 33; // Green
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    myStatusLight.setSolid(PIN_RED); // Red
  } else if (!timeClient.update()) {
    myStatusLight.setSolid(PIN_BLUE); // Blue
  } else {
    myStatusLight.setSolid(PIN_GREEN); // Green
  }
